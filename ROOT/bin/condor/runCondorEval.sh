...
#--------------------------------------------------------
while IFS=, read -r SAMPLE_INFO PROCESS YEAR XSEC NEVENTS
do
    {
    # If stored in eos,
...
while IFS=, read -r SAMPLE_INFO SAMPLE YEAR XSEC NEVENTS
do
    {
    ...
    if [[ ${SAMPLE_INFO} =~ ^/eos/ ]]; then
        find ${SAMPLE_INFO}/${SAMPLE}/${SAMPLE}*.root -type f > ${INPUT_LIST}
        cat ${INPUT_LIST}
    fi
}
done < ${SAMPLES}
